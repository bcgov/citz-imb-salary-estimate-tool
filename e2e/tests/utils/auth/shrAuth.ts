...
import { shrUsername, shrPassword } from '../env/env';

// Function to log in as a SHR user
export async function shrLogin(page: Page) {
  try {
    ...
    // Enter the SHR username
    const userField = page.locator('#user');
    await userField.waitFor({ state: 'visible' });
    await userField.click();
    await userField.fill(shrUsername);

    // Enter the SHR password
    const passwordField = page.getByLabel('Password');
    await passwordField.waitFor({ state: 'visible' });
    await passwordField.click();
    await passwordField.fill(shrPassword);
